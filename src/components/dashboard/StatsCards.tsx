...
export default function StatsCards({ records, settings }: Props) {
  ...
  const totalOTHours = monthRecords.reduce(
    (sum, r) => sum + (r.otHours || 0),
    0,
  );

  // Payroll cutoff logic
  const { current: currentPeriod, next: nextPeriod } = getPayPeriods(
    settings,
    now,
  );
  ...
  const salaryBars = last7.length
    ? last7.map((r) => r.dailyEarnings || 0)
    : [1, 2, 3, 4, 5, 6, 7];
  const hoursBars = last7.length
    ? last7.map((r) => r.hoursWorked || 0)
    : [6, 8, 7, 9, 8, 8, 9];
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: (
        <span>
          {presentRecords.length} of {pastWorkingDays.length} working days
        </span>
      ),
      badge: { label: `${attendancePct}%`, positive: true },
      visual: (
        <MiniBarChart
          values={[3, 5, 4, 6, 5, 7, presentRecords.length || 1]}
          color="#6366f1"
        />
      ),
    },
    {
      ...
      sub: (
        <span>
          {absences} day{absences !== 1 ? "s" : ""} missed this month
        </span>
      ),
      ...
      visual: (
        <MiniProgressBar
          value={absences}
          max={Math.max(pastWorkingDays.length, 1)}
          color="#f59e0b"
        />
      ),
    },
        ...
        {stats.map(({ label, value, sub, badge, visual }) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                className={`flex items-center gap-0.5 text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ml-1 ${
                  badge.positive
            ...
            <div className="flex items-end justify-between gap-2">
              <div className="min-w-0">
                <p className="text-2xl font-extrabold text-gray-900 leading-none truncate">
                  {value}
                </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 text-xs lg:ml-auto opacity-60 lg:opacity-100">
            ...
            <div className="flex flex-wrap items-center gap-1.5 ml-3.5 sm:ml-0">
              ...
              <span className="text-gray-400">
                Pay on {nextPeriod.payDateLabel}
              </span>
              <span className="text-gray-300 mx-1 hidden sm:inline">·</span>
              <span className="text-gray-400">
                {formatCurrency(nextPeriodEarnings, settings.currency)}
              </span>
            </div>
          </div>
        </div>
